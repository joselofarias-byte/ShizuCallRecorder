...
class PhoneNumberManager private constructor(context: Context) {
    ...
    suspend fun parsePhoneNumber(
        rawNumber: String,
        defaultRegion: String = getDeviceCountryIso()
    ): Phonenumber.PhoneNumber? = withContext(Dispatchers.Default) {
        return@withContext try {
            ...
            if (phoneUtil.isValidNumber(parsedNumber)) {
                ...
                AppLogger.v(
                    "Parsed number ($rawNumber) is invalid in region ($defaultRegion), attempting to parse as international: $internationalNumberStr"
                )
    ...
    suspend fun isNumberFromDifferentCountry(
        phoneNumber: Phonenumber.PhoneNumber,
        compareCountryIso: String = getDeviceCountryIso()
    ): Boolean {
